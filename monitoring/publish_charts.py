...
def publish(gangster_root: Path, yakuza_root: Path, output_root: Path) -> int:
    gangster = _read_csv(gangster_root / "games.csv")
    yakuza = _read_csv(yakuza_root / "games.csv")
    changed = 0
    for family in FAMILIES:
...
def main() -> int:
    ...
    parser.add_argument("--yakuza-root", type=Path, required=True)
